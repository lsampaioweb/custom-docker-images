...
COMPONENT_NAME="${COMPONENT_BUILD_NAME}"
CODE_VERSION="21"

# Construct the parent image tag for FROM statement (using dev image).
PARENT_IMAGE_TAG="${CODE_VERSION}-${OS_VARIANT}-${RELEASE_VERSION}"
PARENT_IMAGE_FULLNAME=${REGISTRY_URL}/${BASE_IMAGE_PREFIX}/${COMPONENT_DEV_NAME}:${PARENT_IMAGE_TAG}

# Package to be used in the image.
PACKAGE="maven"
